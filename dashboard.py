...
def format_anomaly_patterns(patterns):
    ...
    for pattern in patterns:
        formatted.append({
            ...
            "Time Range": f"{pattern['time_range']['start']} to {pattern['time_range']['end']}",
            "Duration": f"{pattern['time_range']['duration_seconds']:.1f} sec",
            "Avg Interval": f"{pattern['time_range']['avg_interval']:.1f} sec",
            "Avg Anomaly Score": f"{pattern['score_stats']['avg']:.2f}",
            "Max Score": f"{pattern['score_stats']['max']:.2f}",
            "Min Score": f"{pattern['score_stats']['min']:.2f}"
        })
    return formatted
